    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Manage the services shown on your site</p>
        </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-white/5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <HandCoins className="h-10 w-10 text-slate-600" />
            <p className="mt-3 text-sm text-slate-400">No services yet</p>
            <Link to="/admin/services/new" className="mt-4 text-sm font-medium text-blaze-400 hover:text-blaze-300">
                ...
                <tr key={s.id} className="transition-colors hover:bg-white/5">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${
                      s.status === 'published' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-amber-500/15 text-amber-400'
                    }`}>
                      {s.status}
                    </span>
                  </td>
                  <td className="hidden px-5 py-3.5 text-slate-400 sm:table-cell">{formatDate(s.created_at)}</td>
                  <td className="px-5 py-3.5">
